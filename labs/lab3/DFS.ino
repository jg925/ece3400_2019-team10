...
struct node{
  position pos;
  bool vis;
  direction dir; // which way robot was coming from
};
...
void setup() {
  ...
  int right_wall = digitalRead(right_ir_sensor);
  int left_wall = digitalRead(left_ir_sensor);
  int front_wall = digitalRead(front_ir_sensor);
  direction direct;
  node open1;
  node open2;
